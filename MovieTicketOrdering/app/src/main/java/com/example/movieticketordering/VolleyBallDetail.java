...
public class VolleyBallDetail extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_volley_ball_detail);
        ImageView backbutton =findViewById(R.id.backbtn);
        ImageView poster =findViewById(R.id.volleyballposter);
    ...
    public void onClick(View view){
        ...
        if(view.getId()==R.id.button) {
            GodzillaDetail.SharedViewModel.setTicketTitle("Volley");
            getSupportFragmentManager().beginTransaction().replace(R.id.layout, new OrderFragment()).commit();
        }
        if (view.getId() == R.id.volleyballposter){
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_VIEW);
            intent.addCategory(Intent.CATEGORY_BROWSABLE);
            intent.setData(Uri.parse("https://www.youtube.com/watch?v=gK6RbuM3U7Y&pp=ygUM5o6S55CD5bCR5bm0"));
            startActivity(intent);
            finish();
        }
    }
